...
from . import fsdb_darning_legacy
from . import utils
...
class PatchListData(pm_ifce.PatchListData):
    ...
    def _get_data_text(self, h):
        patches_data = patch_db_legacy.get_patch_table_data()
        selected_guards = patch_db_legacy.get_selected_guards()
...
class Interface(pm_ifce.InterfaceMixin):
    ...
    def __getattr__(attr_name):
        if attr_name == "in_valid_pgnd": return patch_db_legacy.find_base_dir() is not None
        if attr_name == "is_poppable": return patch_db_legacy.get_applied_patch_count() > 0
        if attr_name == "is_pushable": return patch_db_legacy.is_pushable()
        if attr_name == "all_applied_patches_refreshed": return patch_db_legacy.all_applied_patches_refreshed()
    ...
    def create_new_playground(dir_path=None):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_create_db(dir_path), events)
    @staticmethod
    def dir_is_in_valid_pgnd(dir_path=None):
        return patch_db_legacy.find_base_dir(dir_path) is not None
    @staticmethod
    def do_add_files_to_top_patch(file_paths, absorb=False, force=False):
        if absorb:
            ...
            cmd_str = "add {0}\n".format(utils.quoted_join(file_paths))
        cmd_do = lambda: patch_db_legacy.do_add_files_to_top_patch(file_paths, absorb=absorb, force=force)
    ...
    def do_copy_file_to_top_patch(file_path, as_file_path, overwrite=False):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_copy_file_to_top_patch(file_path, as_file_path, overwrite=overwrite), pm_ifce.E_FILE_ADDED)
    @staticmethod
    def do_create_new_patch(patch_name, descr):
        cmd_str = _("new patch {0} --msg \"{1}\"\n").format(utils.quote_if_needed(patch_name), descr)
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_create_new_patch(patch_name, descr), pm_ifce.E_NEW_PATCH|pm_ifce.E_PUSH, False)
    @staticmethod
    def do_delete_files_in_top_patch(file_paths):
        cmd_str = "delete {0}\n".format(utils.quoted_join(file_paths))
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_delete_files_in_top_patch(file_paths), pm_ifce.E_FILE_DELETED)
    @staticmethod
    def do_drop_files_from_patch(file_paths, patch_name=None):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_drop_files_fm_patch(patch_name, file_paths), pm_ifce.E_FILE_DELETED|pm_ifce.E_DELETE_PATCH, False)
    @staticmethod
    def do_duplicate_patch(patch_name, as_patch_name, new_description):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_duplicate_patch(patch_name, as_patch_name, new_description), pm_ifce.E_NEW_PATCH, False)
    @staticmethod
    def do_import_patch(epatch, as_patchname, overwrite=False):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_import_patch(epatch, as_patchname, overwrite=overwrite), pm_ifce.E_NEW_PATCH|pm_ifce.E_PUSH)
    @staticmethod
    def do_fold_epatch(epatch, absorb=False, force=False):
        cmd_str = "fold --file {0}\n".format(utils.quote_if_needed(epatch.source_name))
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_fold_epatch(epatch, absorb=absorb, force=force), pm_ifce.E_FILE_CHANGES)
    @staticmethod
    def do_fold_named_patch(patch_name, absorb=False, force=False):
        cmd_str = "fold --patch {0}\n".format(utils.quote_if_needed(patch_name))
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_fold_named_patch(patch_name, absorb=absorb, force=force), pm_ifce.E_FILE_CHANGES|pm_ifce.E_DELETE_PATCH, False)
    @staticmethod
    def do_move_files(file_paths, destn_path, force=False, overwrite=False):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_move_files_in_top_patch(file_paths, destn_path, force=force, overwrite=overwrite), pm_ifce.E_FILE_ADDED|pm_ifce.E_FILE_DELETED)
    @staticmethod
    def do_pop_top_patch():
        return _RUN_DO("pop\n", lambda: patch_db_legacy.do_unapply_top_patch(), pm_ifce.E_POP, False)
    @staticmethod
    def do_push_next_patch(absorb=False, force=False):
        if absorb:
            ...
            cmd_str = "push\n"
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_apply_next_patch(absorb=absorb, force=force), pm_ifce.E_PUSH, False)
    @staticmethod
    def do_refresh_patch(patch_name=None):
        cmd_str = "refresh {0}\n".format(utils.quote_if_needed(patch_name)) if patch_name else "refresh\n"
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_refresh_patch(patch_name), pm_ifce.E_PATCH_REFRESH)
    @staticmethod
    def do_remove_patch(patch_name):
        cmd_str = "remove patch: {0}\n".format(utils.quote_if_needed(patch_name))
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_remove_patch(patch_name), pm_ifce.E_DELETE_PATCH)
    @staticmethod
    def do_rename_file_in_top_patch(file_path, new_file_path, force=False, overwrite=False):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_rename_file_in_top_patch(file_path, new_file_path, force=force, overwrite=overwrite), pm_ifce.E_FILE_ADDED|pm_ifce.E_FILE_DELETED)
    @staticmethod
    def do_rename_patch(patch_name, new_name):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_rename_patch(patch_name, new_name), pm_ifce.E_MODIFY_PATCH, False)
    @staticmethod
    def do_restore_patch(patch_name, as_patch_name=''):
        ...
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_restore_patch(patch_name, as_patch_name), pm_ifce.E_NEW_PATCH, False)
    @staticmethod
    def do_scm_absorb_applied_patches():
        # notify events regardless of return value as partial success is possible
        return _RUN_DO("absorb\n", lambda: patch_db_legacy.do_scm_absorb_applied_patches(), pm_ifce.E_POP)
    @staticmethod
    def do_select_guards(guards_str):
        cmd_str = "select {0}\n".format(guards_str)
        return _RUN_DO(cmd_str, lambda:patch_db_legacy.do_select_guards(guards_str.split()), pm_ifce.E_MODIFY_GUARDS, False)
    @staticmethod
    def do_set_patch_description(patch_name, descr, overwrite=False):
        cmd_str = "set patch description {0} --msg \"{1}\"\n".format(utils.quote_if_needed(patch_name), descr)
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_set_patch_description(patch_name, descr), pm_ifce.E_MODIFY_PATCH)
    @staticmethod
    def do_set_patch_guards(patch_name, guards_str):
        cmd_str = "set guards {0} {1}\n".format(utils.quote_if_needed(patch_name), guards_str)
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_set_patch_guards_fm_str(patch_name, guards_str), pm_ifce.E_MODIFY_PATCH, False)
    @staticmethod
    def do_set_series_description(text):
        cmd_str = "set series description --msg \"{0}\"\n".format(text)
        return _RUN_DO(cmd_str, lambda: patch_db_legacy.do_set_series_description(text), 0)
    @staticmethod
    def get_applied_patch_count():
        return patch_db_legacy.get_applied_patch_count()
    @staticmethod
    def get_author_name_and_email():
        return None # let ifce handle this
    @staticmethod
    def get_combined_patch_diff_pluses(file_paths=None):
        if patch_db_legacy.get_applied_patch_count() == 0:
            return []
        return patch_db_legacy.get_combined_diff_pluses_for_files(file_paths)
    @staticmethod
    def get_combined_patch_file_db():
        return fsdb_darning_legacy.CombinedPatchFileDb()
    @staticmethod
    def get_extdiff_files_for(file_path, patch_name):
        if patch_name is None:
            patch_name = patch_db_legacy.get_top_patch_for_file(file_path)
        return patch_db_legacy.get_extdiff_files_for(file_path, patch_name)
    @staticmethod
    def get_file_combined_diff(file_path):
        return patch_db_legacy.get_file_combined_diff(file_path)
    @staticmethod
    def get_file_diff(file_path, patch_name):
        return patch_db_legacy.get_file_diff(file_path, patch_name)
    @staticmethod
    def get_filepaths_not_in_patch(patch_name, file_paths):
        return patch_db_legacy.get_filepaths_not_in_patch(patch_name, file_paths)
    @staticmethod
    def get_kept_patch_names():
        return patch_db_legacy.get_kept_patch_names()
    @staticmethod
    def get_named_patch_diff_pluses(patch_name, file_paths=None, with_timestamps=False):
        return patch_db_legacy.get_diff_pluses_for_files(file_paths=file_paths, patch_name=patch_name, with_timestamps=with_timestamps)
    @staticmethod
    def get_outstanding_changes_below_top():
        return patch_db_legacy.get_outstanding_changes_below_top()
    @staticmethod
    def get_patch_description(patch_name):
        return patch_db_legacy.get_patch_description(patch_name)
    @staticmethod
    def get_patch_file_db(patch_name):
        return fsdb_darning_legacy.PatchFileDb(patch_name)
    @staticmethod
    def get_patch_guards(patch_name):
        guards = patch_db_legacy.get_patch_guards(patch_name)
    ...
    def get_playground_root():
        return patch_db_legacy.find_base_dir()
    @staticmethod
    def get_reconciliation_paths(file_path):
        return patch_db_legacy.get_reconciliation_paths(file_path)
    @staticmethod
    def get_selected_guards():
        return patch_db_legacy.get_selected_guards()
    @staticmethod
    def get_series_description():
        return patch_db_legacy.get_series_description()
    @staticmethod
    def get_textpatch(patch_name):
        return patch_db_legacy.get_textpatch(patch_name)
    @staticmethod
    def get_top_patch_diff_pluses(file_paths=None, with_timestamps=False):
        if patch_db_legacy.get_applied_patch_count() == 0:
            return []
        return patch_db_legacy.get_diff_pluses_for_files(file_paths=file_paths, patch_name=None, with_timestamps=with_timestamps)
    @staticmethod
    def get_top_patch_file_db():
        return fsdb_darning_legacy.TopPatchFileDb()
    @staticmethod
    def get_top_patch_for_file(file_path):
        return patch_db_legacy.get_top_patch_for_file(file_path)
    @staticmethod
    def is_blocked_by_guard(patch_name):
        return patch_db_legacy.is_blocked_by_guard(patch_name)
    @staticmethod
    def is_patch_applied(patch_name):
        return quilt_utils.is_patch_applied(patch_name)
    @staticmethod
    def is_top_patch(patch_name):
        return patch_db_legacy.is_top_patch(patch_name)
