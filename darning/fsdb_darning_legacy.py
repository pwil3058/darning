...
_STATUS_DECO_MAP = {
    ...
    patch_db_legacy.Presence.ADDED: fsdb.Deco(Pango.Style.NORMAL, "darkgreen"),
    patch_db_legacy.Presence.REMOVED: fsdb.Deco(Pango.Style.NORMAL, "red"),
    patch_db_legacy.Presence.EXTANT: fsdb.Deco(Pango.Style.NORMAL, "black"),
}
...
class FileData(fsdb.FileData):
    ...
    def icon(self):
        ...
        if status.validity == patch_db_legacy.Validity.REFRESHED:
            return self.icons.STOCK_FILE_REFRESHED
        elif status.validity == patch_db_legacy.Validity.NEEDS_REFRESH:
            return self.icons.STOCK_FILE_NEEDS_REFRESH
        elif status.validity == patch_db_legacy.Validity.UNREFRESHABLE:
            return self.icons.STOCK_FILE_UNREFRESHABLE
        else:
...
class _PatchFileDir(fsdb.GenericChangeFileDb.FileDir):
    ...
    def _calculate_status(self):
        if not self._status_set:
            validity = patch_db_legacy.Validity.REFRESHED
        else:
            validity = max([s.validity for s in list(self._status_set)])
        return patch_db_legacy.FileStatus(None, validity)
...
class TopPatchFileDb(fsdb.GenericTopPatchFileDb):
    ...
    def _get_patch_data_text(h):
        patch_status_text = patch_db_legacy.get_patch_file_table()
...
class CombinedPatchFileDb(TopPatchFileDb):
    def _get_patch_data_text(self, h):
        patch_status_text = patch_db_legacy.get_combined_patch_file_table()
        h.update(str(patch_status_text).encode())
        return patch_status_text
...
class PatchFileDb(fsdb.GenericPatchFileDb):
    ...
    def _get_is_applied(patch_name):
        return patch_db_legacy.is_patch_applied(patch_name)
    def _get_patch_data_text(self, h):
        patch_status_text = patch_db_legacy.get_patch_file_table(self.patch_name)
        h.update(str(patch_status_text).encode())
        return patch_status_text
