...
class WSTreeView(file_tree.FileTreeView):
    ...
    def __init__(self, busy_indicator=None, show_hidden=False, hide_clean=False):
        file_tree.FileTreeView.__init__(self, busy_indicator=busy_indicator, show_hidden=show_hidden, hide_clean=hide_clean, popup="/pmic_files_popup")
        ...
        self.action_groups[ws_actions.AC_IN_PM_PGND_MUTABLE + ws_actions.AC_PMIC + actions.AC_SELN_MADE].add_actions(
            [
                ('pm_add_files_to_top_patch', gtk.STOCK_ADD, _('_Add'), None,
                 _('Add the selected files to the top patch'), lambda _action=None: self.pm_add_selected_files()),
                ('pm_edit_files_in_top_patch', gtk.STOCK_EDIT, _('_Edit'), None,
                 _('Open the selected files for editing after adding them to the top patch'), lambda _action=None: self.pm_edit_selected_files()),
                ('pm_delete_files_in_top_patch', gtk.STOCK_DELETE, _('_Delete'), None,
                 _('Add the selected files to the top patch and then delete them'), lambda _action=None: self.pm_delete_selected_files()),
            ])
