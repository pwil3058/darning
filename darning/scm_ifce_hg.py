...
class Mercurial(object):
    ...
    def do_backout(self, rev, msg, merge=False):
        ...
        ws_event.notify_events(scm_ifce.E_BACKOUT|scm_files.E_FILE_CHANGES)
        return result
    @staticmethod
    def do_clone_as(dir_path, target=None):
        cmd = ["hg", "clone", dir_path] + OPTNL_ARG(target)
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_CLONE)
        return result
    @staticmethod
    def do_commit_change(self, msg, file_paths=None, amend=False):
        ...
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_COMMIT|scm_ifce.E_FILE_CHANGES)
        return result
    @staticmethod
    ...
    def do_merge_workspace(rev=None, force=False):
        ...
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_MERGE)
        return result
    @staticmethod
    def do_move_files(file_paths, destn, force=False, dry_run=False):
        ...
        if dry_run:
            return runext.run_cmd(cmd + ["-n", "--verbose"] + file_paths + [destn]).mapped_for_suggestions(SUGGESTION_TABLE)
        result = _run_do_cmd(cmd + file_paths + [destn])
        ws_event.notify_events(scm_ifce.E_FILE_DELETED|scm_ifce.E_FILE_ADDED)
        return result
    @staticmethod
    def do_move_tag(tag, rev, msg=None):
        cmd = ["hg", "tag", "-f", "--rev", rev] + OPTNL_FLAG_WITH_ARG("-m", msg) + [tag]
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_TAG)
        return result
    @staticmethod
    def do_pull_from(rev=None, update=False, source=None):
        ...
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_PULL|scm_ifce.E_UPDATE if update else scm_ifce.E_PULL)
        return result
    @staticmethod
    def do_push_to(rev=None, update=False, path=None):
        cmd = ["hg", "push"] + OPTNL_FLAG(update, "-u") + OPTNL_FLAG_WITH_ARG("--rev", rev) + OPTNL_ARG(path)
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_PUSH|scm_ifce.E_UPDATE if update else scm_ifce.E_PUSH)
        return result
    @staticmethod
    def do_remove_files(file_paths, force=False):
        cmd = ["hg", "remove"] + OPTNL_FLAG(force, "-f") + file_paths
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_FILE_DELETED)
        return result
    @staticmethod
    ...
    def do_rollback_repo():
        ...
        ws_event.notify_events(scm_ifce.E_CS_CHANGES|scm_ifce.E_WD_CHANGES)
        return result
    @staticmethod
    def do_set_branch(branch, force=False):
        cmd = ["hg", "branch"] + OPTNL_FLAG(force, "-f") + [branch]
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_BRANCH)
        return result
    @staticmethod
    ...
    def do_update_workspace(rev=None, discard=False):
        ...
        result = _run_do_cmd(cmd)
        ws_event.notify_events(scm_ifce.E_UPDATE)
        return result
    @staticmethod
    def do_verify_repo():
